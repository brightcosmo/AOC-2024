...
def solve(s: str) -> int:
    ...
    # SOLUTION
    # Same as before, but with new CONCAT operator (||) where 15 || 6 = 156
    lines = [
...
def process(target: int, operands: List[int]) -> int:
    ...
    str_total = str(target)
    str_ops = str(curr_operand)
    
    if (len(str_total) > len(str_ops) and str_total.endswith(str_ops)):
        reduced_total = int(str_total[:-len(str_ops)])
        if process(reduced_total, operands.copy()):
            return True
