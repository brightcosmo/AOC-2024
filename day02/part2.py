...
def solve(s: str) -> int:
    ...
    return sum([check_tolerant_safety(seq) for seq in sequences])

def check_safety(seq: List[int]) -> bool:
    diffs = [seq[i+1] - seq[i] for i in range(len(seq) - 1)]
    return (max(diffs) <= 3 and min(diffs) >= 1) or (max(diffs) <= -1 and min(diffs) >= -3)

def check_tolerant_safety(seq: List[int]) -> bool:
    return any(check_safety(seq[:i]+seq[i+1:]) for i in range(len(seq)))
